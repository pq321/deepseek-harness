...
折叠搜索是视图和添加操作旁的一枚区头按钮。在轨道中，添加和搜索会渲染为沿外壳共用横向进入路径移动的 36px 控件。激活搜索后，一个 VS Code 风格的输入框会扩展并占据区头，提供**区分大小写**、**全字匹配**和**使用正则表达式**开关；点击外部只会收起经清除首尾空白后为空的查询——但轨道搜索手势仍在进行期间（直至列滑动结束、焦点落入输入框）除外，这样触发展开的那次点击不会收起它刚打开的搜索——而清除控件总会重置并收起搜索。非空白查询会以单一扁平结果列表替代任一浏览模式，标题和 Workspace 会按照所选匹配选项立即产生结果。少于三个 Unicode 字符的查询只在本地匹配；更长的查询会在最后一次按键后等待 350 ms，再向 Host 请求经过排序的当前对话内容匹配项及其摘要片段。无效正则表达式会直接在输入框处被拒绝，不会联系 Host。搜索输入框及其防御性请求路径会移除 NUL，将查询限制在传输 schema 规定的 500 个 UTF-16 代码单元内且不会拆分代理项对。每次新查询或选项变更都会中止前一个请求；内容搜索失败时，元数据匹配项仍会显示，同时给出警告。列表最多显示 20 条结果，并会在查询过宽时提示用户缩小范围。选择内容结果会保留查询、打开对应 Session，并把匹配 `eventSeq` 交给会话服务以精确定位行；仅匹配标题或 Workspace 的结果则正常打开。
...
- **没有拼写容错搜索**：系统提供部分字面、区分大小写、全字和正则表达式模式，但不会纠正拼写错误；只有内容结果带有事件地址，因此仅匹配标题或 Workspace 的结果无法精确定位行。
