...
`ctx.conversation` 以一份包含 Session id 和 `eventSeq`、按身份校验的临时聚焦请求承载精确消息导航。聊天视图只接受属于自身 Session 的请求，每次加载一页更早历史，直到目标节点进入投影；随后把该行滚动到中央，并显示 2.4 秒的搜索高亮。用户偏好减少动态效果时会停用平滑滚动。请求会在目标显现后、历史耗尽仍未找到事件时，或所属 Session 视图卸载时被消费；较旧消费者无法清除较新的请求。
...
提交的用户消息会增加一个末尾 `Referenced files (content not uploaded)` 文本段，其中包含每个浏览器可见的文件名或相对路径。文件内容不会被读取或上传。

#### Token 影响

每个引用都会把固定文本段标题和经 JSON 转义的路径或文件名加入会话上下文。

#### KV Cache 影响

引用文本与普通提示词文本一样进入用户消息前缀。文件字节从不进入请求，因此没有缓存影响。
