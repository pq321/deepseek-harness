...
`searchSessions(request, exec?)` returns `SessionSearchHit` pages across the corpus; `searchEvents(request, exec?)` returns `SessionEventSearchHit` pages within one session. Queries are required and trimmed. The default mode whitespace-normalizes the query and searches one case-insensitive literal phrase whose final token may be a prefix, so `spatiotempora` matches `spatiotemporal`; `matchWholeWord` removes that prefix expansion. `matchCase` verifies indexed candidates against original document text. `useRegularExpression` preserves internal whitespace, compiles the query as a Unicode JavaScript regular expression, and scans original indexed document text; `matchCase` controls its `i` flag and `matchWholeWord` adds Unicode letter, number, mark, and underscore boundaries. Invalid expressions fail with `SESSION_QUERY_INVALID_QUERY`. FTS5 syntax in literal mode remains inert data. Metadata filters are parameterized SQL predicates applied before ranking. To keep SQLite FTS5 MATCH in a supported outer-predicate context, cross-session requests may compile at most 14 combined session and event filter predicates; within-session requests may compile at most 13 filter predicates because the fixed target-session predicate consumes one slot. Each range endpoint compiles as one predicate. A request exceeding either predicate budget or SQLite's portable limit of 32,766 total bindings, including fixed query and pagination values, fails with `SESSION_QUERY_INVALID_FILTER` before statement preparation.
...
The default indexed mode uses FTS5 `unicode61`: it expands only the final token prefix, so `AI` still does not match inside `BRAID`. Regular-expression mode can express arbitrary substrings but scans original indexed document text synchronously and is therefore more expensive. NUL is rejected in queries; reserved highlight markers and NUL in documents are normalized before indexing so presentation markers cannot collide with source text.
...
- **Regular-expression cost** — regular-expression mode scans original indexed documents through synchronous `DatabaseSync` execution and can block the JavaScript thread longer than indexed literal searches.
