...
**profile** 是存放在 Harness home 中的具名组装。它列出自己叠放的组合包，存放自己安装的树外插件，保存用户自己的 `cordis.patch.yml`，还可以在 `dsh.profile.entryStates` 下持久化由浏览器管理的 Loader 条目启用状态。`web` 和 `headless` 作为模板随发行版交付。
...
各层按此顺序应用在空条目列表之上：先按 profile 列出的顺序应用每个组合包，然后是 profile 的 `cordis.patch.yml`，然后是 home 级的那份，再应用 profile `entryStates`，最后是任意 `--patch` overlay。一条 patch 按 id 定位某个条目并替换其整个 config，或插入新条目；`entryStates` 只为对应 Loader id 贡献已保存的 `disabled` 字段。
