...
- The escalation fields exist exactly when the mounted executor confines; a target equal to the call's effective mode executes idempotently without approval, a narrower or unknown target fails closed without approval, and a deployment with no ApprovalService fails strictly wider calls closed while leaving plain and equal-mode calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: execution treats an equal target as the standing mode without approval, while the strict-wider check rejects narrower or unknown targets and routes only a wider target to approval.
